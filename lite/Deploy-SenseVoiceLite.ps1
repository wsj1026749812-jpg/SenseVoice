...
$server = Join-Path $packageRoot "SenseVoiceLite.Server.exe"
$pidPath = Join-Path $packageRoot "SenseVoiceLite.Server.pid"
...
if (-not (Test-Path -LiteralPath $server)) {
    throw "SenseVoice Lite server was not found: $server"
}
...
if (Test-Path -LiteralPath $pidPath) {
    ...
    if ($existingProcess) {
        if (-not $Replace) {
            throw "SenseVoice Lite is already running (PID $existingPid). Use -Replace to restart it."
        }
...
do {
    try {
        ...
        if ($health.status -eq "ok") {
            Write-Host "SenseVoice Lite is ready on CPU: $healthUrl"
            return
        }
    }
    catch {
...
throw "SenseVoice Lite did not become ready. See $stderrLog"
