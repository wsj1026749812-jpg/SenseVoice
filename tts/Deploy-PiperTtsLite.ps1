...
$server = Join-Path $packageRoot "PiperTtsLite.Server.exe"
$pidPath = Join-Path $packageRoot "PiperTtsLite.Server.pid"
...
if (-not (Test-Path -LiteralPath $server)) {
    throw "Piper TTS Lite server was not found: $server"
}
...
if (Test-Path -LiteralPath $pidPath) {
    ...
    if ($existingProcess) {
        if (-not $Replace) {
            throw "Piper TTS Lite is already running (PID $existingPid). Use -Replace to restart it."
        }
...
do {
    try {
        ...
        if ($health.status -eq "ok") {
            Write-Host "Piper TTS Lite is ready on CPU: $healthUrl"
            return
        }
    }
    catch {
...
throw "Piper TTS Lite did not become ready. See $stderrLog"
